...
- [x] **Security Command Center: no activable en este entorno** — GCP exige pertenecer a una **Organización** y el proyecto vive en una cuenta personal sin ella (verificado en consola: `security/command-center/no-org`). Limitación documentada para el reporte; el dominio de postura de seguridad se cubre con los equivalentes implementados (dashboard de golden signals, detección por flow logs demostrada, CVE scanning, audit logs) y la adopción de SCC queda en el roadmap de madurez condicionada a migrar a una organización.
