...
deploy_orders() {
  gcloud run deploy orders-api \
    ...
    --set-env-vars "INVENTORY_URL=$INVENTORY_URL,OTEL_SERVICE_NAME=orders-api,OTEL_EXPORTER_OTLP_PROTOCOL=grpc,OTEL_METRIC_EXPORT_INTERVAL=5000,OTEL_EXPORTER_OTLP_ENDPOINT=$OTEL_ENDPOINT" \
    --port 8080 \
...
deploy_inventory() {
  gcloud run deploy inventory-api \
    ...
    --set-env-vars "OTEL_SERVICE_NAME=inventory-api,OTEL_EXPORTER_OTLP_PROTOCOL=grpc,OTEL_METRIC_EXPORT_INTERVAL=5000,OTEL_EXPORTER_OTLP_ENDPOINT=$OTEL_ENDPOINT" \
    --port 8081 \
    --allow-unauthenticated \
    --quiet
}
